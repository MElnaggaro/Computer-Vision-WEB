...
def write_frontend_config(api_base_url: str) -> None:
    ...
    # Cadence picked to keep the backend quiet while still feeling
    # real-time on screen.  Each value is a deliberate compromise:
    #   • RECOGNIZE_INTERVAL_MS — minimum gap between recognize-frame
    #     POSTs.  The dashboard self-paces: it only fires the next
    #     request AFTER the previous response, so this is a *floor*,
    #     not a ceiling.  200 ms = 5 fps when the backend is fast.
    #   • EVENT_POLL_MS — `/logs/events?since=cursor` poll for new
    #     attendance/question events.  recognize-frame already
    #     short-circuits a poll on `newly_marked`, so this is just the
    #     idle-refresh tick.  1500 ms keeps the panel snappy without
    #     hammering the server.
    #   • HEALTH_INTERVAL_MS — `/health` ping when the backend is
    #     reachable.  We only need this slow heartbeat to detect the
    #     backend going offline; on offline the dashboard automatically
    #     falls back to a fast retry (see dashboard.js).
    config_payload = (
        ...
        "    HEALTH_OFFLINE_INTERVAL_MS: 2000,\n"
        "    RECOGNIZE_INTERVAL_MS: 200,\n"
        "};\n"
    )
    try:
